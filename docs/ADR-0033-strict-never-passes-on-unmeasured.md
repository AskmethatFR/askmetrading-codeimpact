...
- **#147 (HIGH) — un chemin commençant par `.` sort entièrement du gate.** `hidden(true)` (`file_system_code_reader.rs:346`) écartait l'entrée **sans aucune trace, sur aucune surface** : ni `dropped_files`, ni `unexplored_subtree`, ni JSON, ni HTML, ni console. Security a démontré une charge réellement dépassante de **1 600 fonctions** s'échappant par un simple renommage de répertoire : `heavy/` → complexité 3 241, exit **3** ; `.heavy/` → complexité 1, exit **0 silencieux**. Représentable en git, **survivant à un clone frais**. Préexistant (#83), pas introduit ici. **Clos par #147** (voir [[ADR-0006]]) : le saut reste (un `hidden(false)` sec ingérerait `.git/`, `.venv/`), mais il est **compté** — `hidden_excluded_files_count` sur les trois surfaces.
- **#145 — `exclude` réduit l'ensemble mesuré sans trace exploitable par machine.** `.codeimpact.json` ([[ADR-0019]]) peut rétrécir le périmètre ; `default_excluded_count` ne comptait que le sous-ensemble **par défaut**, pas les motifs de l'utilisateur. Cela **défait la remédiation documentée d'[[ADR-0006]]** § *Frontière de confiance* : les flags `--max-kwh`/`--max-co2` surclassent bien les **seuils**, mais `exclude` **n'a aucun équivalent CLI et aucun surclassement**. **Clos par #147 (Volet B)** : `user_excluded_files_count` compte les motifs du dépôt sur les trois surfaces ; le surclassement CLI reste absent (assumé).
- **#146 — un nom de fichier hostile peut masquer visuellement l'avertissement.** Un attribut SGR laissé actif peut dissimuler la ligne de couverture incomplète. **Clos par #147 (Volet C)** : `sanitize_console_text` a été déplacée dans l'hexagone (`console_sanitize`, ré-exportée par `humanize.rs`) et les `eprintln!` nommant un fichier passent par elle.
...
- ~~**#147 (HIGH)** — `hidden(true)` hors du signal de couverture.~~ **Clos par #147** : les entrées `.`-préfixées restent écartées (un `hidden(false)` sec ingérerait `.git/`, `.venv/`) mais sont désormais **comptées** (`hidden_excluded_files_count`, les trois surfaces) — voir [[ADR-0006]] § *Frontière de confiance* et l'invariant commun qui y est énoncé.
- ~~**#145** — `exclude` de `.codeimpact.json` sans trace machine ni surclassement CLI.~~ **Clos par #147 (Volet B)** : `user_excluded_files_count` compte séparément les motifs du dépôt et le porte sur les trois surfaces. Le **surclassement CLI** d'`exclude` reste absent (assumé, CODEOWNERS couvre l'intention).
- ~~**#146** — assainissement console inatteignable depuis l'hexagone sans inverser la règle de dépendance.~~ **Clos par #147 (Volet C)** : `sanitize_console_text` a été déplacée dans l'hexagone (`console_sanitize`), `humanize.rs` la ré-exporte, et les `eprintln!` nommant un fichier (domaine et marcheur) passent par elle.
