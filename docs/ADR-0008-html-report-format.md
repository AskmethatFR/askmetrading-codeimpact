...
**Status:** Applied  
**Date:** 2026-07-11 (amended 2026-07-12 — #27: ADR-8.10, ADR-8.11, addenda 8.8a/8.8b; ADR-8.7's font clause superseded; amended 2026-07-20 — #33/T3: ADR-8.12 support/note + `SUP` whitelist; amended 2026-07-23 — #28: ADR-8.13 gate hardening + runtime jsdom regression test)  
**Decided in:** #7, #27, #33, #28  
**Relations:**  
  supersedes: []  
...
| ADR-8.10 | **The tree, aggregation, thresholds, bar percentages and number formatting are computed in Rust** (`html/view_model.rs`); the JS is a pure `VM → DOM` projection. Rendering discipline: one `el()` node factory (`textContent` only); colours as CSS classes resolved through closed whitelists (`hasOwnProperty` + fallback), never style strings built from data; **exactly two** clamped numeric `.style` sinks; total ban on `innerHTML`/`outerHTML`/`insertAdjacentHTML`/`document.write`/`setAttribute`/`eval`/`new Function`/`javascript:`/`srcdoc`/`cssText`; handlers only via `addEventListener` + closure | This is the **enforceable form of ADR-8.5/8.6 for a dynamic DOM**. T2's DOM is ~10× bigger with far more interpolation sites, so "remember to use `textContent`" stops being a defense. The smaller and dumber the JS, the more auditable the boundary — and pushing the arithmetic into Rust puts it under tests that bite. The reference design markup interpolates into `style="…"` everywhere: a sink T1 did not have, deliberately not ported. Pinned by two structural tests, hardened by ADR-8.13. |
| ADR-8.13 | **Addendum (#28) — the two ADR-8.10 structural gates are hardened against concatenation/bracket-notation bypasses, plus a genuine runtime jsdom regression test** | The #27 Security audit found the gates matched literal substrings only — a sink reached via `"inner" + "HTML"` concatenation or `node["style"][computedProp]` bracket notation never tripped them (not a shipped vuln: the real JS used neither pattern — but a bypassable gate reads as "covered", which is worse than none). `rendering_gate` (shared by both the "real JS passes" and "a known bypass fails" tests) now also catches: a banned identifier reconstructed via string-literal concatenation; bracket-notation style access; `Object.assign(` bulk-setting any property in one call; a bare `.style =` reassignment. Because a static text scan can never prove DOM-level behavior, `html_renderer_runtime_security_test.rs` additionally executes the real emitted JS inside **jsdom** (a genuine parser/DOM, dev-only Node dependency — no `Cargo.toml`, never on the hexagon/secondaries dependency graph) against a hand-patched adversarial data island, asserting no markup parsing, no code execution, `Object.prototype` hygiene, and that hostile whitelist keys / numeric values hit their documented fallback and clamp. Verified to genuinely discriminate by temporarily removing `cls()`'s `hasOwnProperty` guard and `setPct`'s `isFinite` clamp — both turned the corresponding assertions red, then reverted. Node's absence (or a too-old Node — jsdom's own floor is `^20.19.0 \|\| ^22.13.0 \|\| >=24.0.0`) fails the test LOUDLY via `panic!` (#45 lesson: a guard that silently no-ops is worthless), never a quiet skip; CI pins Node 22 via `actions/setup-node` so this is a local-dev-only concern. **Vehicle: Node/jsdom over an in-Rust engine (boa/quickjs).** A hand-rolled DOM shim wrapped around boa/quickjs could only validate its own assumptions about HTML parsing — "no markup was parsed" is a claim only falsifiable against a real, spec-conformant HTML parser, which jsdom genuinely is and a hand-rolled shim is not; the dev-only Node dependency (never on the `Cargo.toml`/hexagon/secondaries graph) buys that independence. **Known residuals (recorded so a future maintainer inherits them instead of rediscovering them):** (1) the STATIC gate is a text scanner — a dynamically-constructed sink identifier (e.g. `node[String.fromCharCode(105,110,110,101,114,72,84,77,76)]`) structurally evades it; accepted (diminishing returns — nobody writes that unobfuscated by accident, and Dev-B review would flag it). (2) the RUNTIME jsdom test's adversarial data path is SCOPED — it patches the root node's `level`/`metrics` and the tree's `name`/`path`, but does not feed hostile data through `renderWarnings`/`renderIo`/`renderFunctions`/`renderImpact`/`renderThresholds`/`renderUnmeasurable`; a future sink hidden in one of those under-exercised paths could evade both layers simultaneously. Mitigant: all text funnels through the shared `el()`/`cls()` (rules 1/3/4 of ADR-8.10), so the verified mechanism is common code, not per-callsite — residual risk low, not absent. |
