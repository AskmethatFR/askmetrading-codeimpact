...
> **Links:** [[architecture-overview]], [[ADR-0004]], [[ADR-0006]], [[economic-impact-estimator]], [[ADR-0015]], [[ADR-0016]], [[ADR-0017]]
...
- **(+) (#62)** Une source hostile trop volumineuse (> 1 MB) est refusée à la mesure comme `Unmeasurable(SourceTooLarge)` **avant** `syn::parse_file` — jamais un crash, jamais un `0` silencieux, exactement la mécanique #50 (le fichier apparaît NON MESURÉ, exclu des sommes). Voir ADR-0006 §2. **(#53)** la cible de sortie non-régulière (symlink/FIFO/…) est refusée avant écriture (ADR-0006 §4).
...
- **#63 (non résolu, en cours) — DoS par stack overflow dans `syn::parse_file`.** Un fichier ~10 KB (mods imbriqués, `Vec<Vec<…>>`, `!!!!…`, `&mut …` en chaîne) déborde la pile → `SIGABRT` **non rattrapable** (`?` ne le voit pas, `catch_unwind` non plus, un thread à pile bornée abort quand même le parent). Une première tentative — un **pre-scan par énumération de caractères** (compter `{([` + runs de `&`) — a été **rejetée par la barrière Security + Dev-B : 3 bypass en une passe** (chevrons non comptés, `!` non compté, lettres entre `&` resettant le compteur), tous reproduits end-to-end. **Même leçon que le §« différence de nature vs marge » ci-dessus** : un scan qui *énumère* des vecteurs se fait toujours franchir ; il faut une garde qui ferme la *classe*. Le pre-scan a été **retiré** (ne pas shipper une protection illusoire + faux positifs sur JSON-en-raw-string). Fix retenu pour le cycle dédié : **isolation sous-processus** de `syn::parse_file` (exit par signal → `Unmeasurable` pour ce fichier seul). Suivi : #63.
- **#63 vs #52 — pourquoi deux tickets, pas une mutualisation (AC4, tranché par l'Architecte).** Même famille de dette (récursion non bornée sur une entrée hostile) mais deux surfaces disjointes : #63 est la récursion **de `syn::parse_file` lui-même**, dans l'**enfant** canari, fermée par l'isolation sous-processus ci-dessus. #52 est la récursion **du graphe d'appels agrégé** (`dfs_reachable`, `tarjan_scc`, `compute_depth`), qui tourne dans le **parent**, après que tous les fichiers ont déjà été parsés individuellement — l'isolation par sous-processus de #63 ne la couvre pas (le graphe n'existe qu'une fois les probes terminées). #52 reste donc un ticket dédié.
